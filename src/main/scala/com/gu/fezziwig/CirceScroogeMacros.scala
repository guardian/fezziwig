...
private class CirceScroogeMacrosImpl(val c: blackbox.Context) {
    ...
    val params = apply.paramLists.head.zipWithIndex.map { case (param, i) =>
      ...
      val decodeExpr = {
        ...
        if (param.asTerm.isParamWithDefault) {
          ...
          // TODO - consider whether this is the correct behaviour. We should probably only use the default value if an optional value is not present
      ...
      val accDecodeExpr = {
        ...
        val accDecodeParam =
          q"""cursor.downField(${name.toString}).success
            .map(x => $implicitDecoder.accumulating(x)).getOrElse(_root_.cats.data.Validated.invalidNel(DecodingFailure($errMsg, cursor.history)))"""

        if (param.asTerm.isParamWithDefault) {
          val defaultValue = A.companion.member(TermName("apply$default$" + (i + 1)))
          q"""$accDecodeParam.orElse(_root_.cats.data.Validated.valid[_root_.cats.data.NonEmptyList[_root_.io.circe.DecodingFailure],$tpe]($defaultValue))"""
        } else {
          q"""$accDecodeParam"""
        }
      }
  ...
  def decodeThriftUnion[A: c.WeakTypeTag]: c.Tree = {
    ...
    val decoderCases: (List[Tree],List[Tree]) = memberClasses.toList.foldLeft(List[Tree](), List[Tree]()) { (acc, memberClass) =>
      ...
      val accDecExpr = {
        val implicitDecoderForParam: c.Tree = getImplicitDecoder(paramType)
        val errMsg = s"Unable to find value for $paramName"
        cq"""$paramName =>
          c.downField($paramName).success.map(x => $implicitDecoderForParam.accumulating(x).map($applyMethod))
            .getOrElse(_root_.cats.data.Validated.invalidNel(DecodingFailure($errMsg, c.history)))"""
      }
